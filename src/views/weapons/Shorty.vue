<template>
    <v-container class="mb-16">
      <b-breadcrumb :items="Shorty" class="mt-5 mb-10"></b-breadcrumb>
      <div class="d-flex justify-content-between align-center flex-wrap">
        <h2 class="mb-0">Shorty</h2>
  ...
  <script>
  import { Shorty } from "@/data/Breadcrump";
        ...
        try {
          ...
          if (res.data.status === 200) {
            const filteredData = res.data.data.filter(
              (item) => item.displayName.includes("Shorty") && item.displayIcon != null
            );
