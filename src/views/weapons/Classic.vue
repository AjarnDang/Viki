<template>
    <v-container class="mb-16">
      <b-breadcrumb :items="Classic" class="mt-5 mb-10"></b-breadcrumb>
      <div class="d-flex justify-content-between align-center flex-wrap">
        <h2 class="mb-0">Classic</h2>
  ...
  <script>
  import { Classic } from "@/data/Breadcrump";
        ...
        try {
          ...
          if (res.data.status === 200) {
            const filteredData = res.data.data.filter(
              (item) => item.displayName.includes("Classic") && item.displayIcon != null
            );
